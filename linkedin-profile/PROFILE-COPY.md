# Ready-to-paste profile copy

Everything below is written from **verified** business facts (your email signature,
your AHCA license, and your public listings). Nothing here is invented.

If a detail changes, change it here first so every place stays consistent.

> **All details confirmed by the owner.** Nothing below is a placeholder.
>
> One open option: **AHCA #9102 is the Hialeah Gardens license.** The Hialeah
> center operates as *Xclusive Senior Day Care Center II, Inc* and has its own
> license number. The license is therefore stated next to the location it belongs
> to, rather than over both. If you want Center II's number shown as well, add it
> in the same spots — the facts table, both About sections, and the Experience
> entry.

---

## The facts everything is built from

| Field | Value |
|---|---|
| Legal name | XCLUSIVE SENIOR DAY CARE CENTER, INC |
| Public / brand name | Xclusive Senior Care |
| License | AHCA #9102 |
| Locations | **Two centers** |
| Location 1 | Xclusive Senior Day Care Center, Inc — 12975 W. Okeechobee Rd., Units 2, 3, 4, Hialeah Gardens, FL 33018 · (305) 820-0805 · AHCA #9102 |
| Location 2 | Xclusive Senior Day Care Center II, Inc — 705 E 8th Ave., Hialeah, FL 33010 · (305) 320-9820 |
| Phone | (305) 820-0805 |
| Fax | (305) 402-8509 |
| Email | info@xclusiveseniorcare.com |
| Website | xclusiveseniorcare.com |
| Hours | Monday–Friday, 9:00 AM – 5:00 PM |
| Languages | English and Spanish — 100% bilingual |

---

## 1. Name fields

The profile currently has the word **"undefined"** in it, which is why the URL reads
`.../xclusive-senior-day-center-undefined-293040433`. That happened because the
sign-up flow never received a last name. Fix it first — it is the single most
unprofessional thing on the page.

| Field | Type this |
|---|---|
| First name | `Xclusive Senior` |
| Last name | `Day Care Center` |

---

## 2. Headline

*(LinkedIn limit: 220 characters. This one is 185.)*

```
Licensed Adult Day Care | 2 Locations in Hialeah & Hialeah Gardens, FL | 100% Bilingual | RN On Site | Meals & Transportation Included | $0 with Medicaid Long-Term Care | (305) 820-0805
```

---

## 3. About section — English

*(LinkedIn limit: 2,600 characters. This one is about 1,460.)*

```
Xclusive Senior Care is a licensed adult day care center with two locations, serving families across Hialeah and Hialeah Gardens, Florida.

Every weekday we give older adults a place to spend the day with purpose, company and professional supervision — and we give their families the confidence to go to work knowing someone they trust is watching over their loved one.

WHAT A DAY WITH US LOOKS LIKE
- A registered nurse on site, with supervised medication
- One caregiver for every four seniors
- Therapeutic programs and daily physical exercise
- Life enrichment: art, music and horticulture
- Leisure activities: hobbies, crafts and outings
- A hot USDA-approved noon meal, plus morning and afternoon snacks
- Door-to-door transportation, included

WHAT IT COSTS
Most of the families we serve pay $0 through Medicaid Long-Term Care. If you are not sure what your plan covers, call us and we will check it for you.

100% BILINGUAL
Our whole team speaks English and Spanish. No family is handed a form they cannot read, and no family is left out of a conversation about their own parent.

OUR TWO LOCATIONS
- Hialeah Gardens: 12975 W. Okeechobee Rd., Units 2-4, Hialeah Gardens, FL 33018 - (305) 820-0805 - AHCA #9102
- Hialeah: 705 E 8th Ave., Hialeah, FL 33010 - (305) 320-9820

Monday to Friday, 9:00 AM to 5:00 PM
(305) 820-0805 | info@xclusiveseniorcare.com | xclusiveseniorcare.com

Come see either center before you decide. Call us to schedule a tour.
```

---

## 4. About section — Español

Use this with LinkedIn's **"Add profile in another language"** (see step 8 of the
README), which is the correct way to run a bilingual profile — not by pasting both
languages into one box.

```
Xclusive Senior Care es un centro de cuidado diurno para adultos con licencia, con dos centros al servicio de las familias de Hialeah y Hialeah Gardens, Florida.

Cada día de semana les damos a los adultos mayores un lugar donde pasar el día con propósito, compañía y supervisión profesional — y les damos a sus familias la tranquilidad de ir a trabajar sabiendo que alguien de confianza cuida a su ser querido.

CÓMO ES UN DÍA CON NOSOTROS
- Enfermera registrada en el centro, con medicación supervisada
- Un cuidador por cada cuatro adultos mayores
- Programas terapéuticos y ejercicio físico diario
- Enriquecimiento: arte, música y horticultura
- Actividades recreativas: pasatiempos, manualidades y paseos
- Almuerzo caliente aprobado por USDA, más meriendas por la mañana y por la tarde
- Transporte puerta a puerta, incluido

CUÁNTO CUESTA
La mayoría de las familias que atendemos pagan $0 a través de Medicaid Long-Term Care. Si no está seguro de lo que cubre su plan, llámenos y lo verificamos por usted.

100% BILINGÜE
Todo nuestro equipo habla español e inglés. Ninguna familia recibe un formulario que no pueda leer, ni queda fuera de una conversación sobre su propio padre o madre.

NUESTROS DOS CENTROS
- Hialeah Gardens: 12975 W. Okeechobee Rd., Units 2-4, Hialeah Gardens, FL 33018 - (305) 820-0805 - Licencia AHCA #9102
- Hialeah: 705 E 8th Ave., Hialeah, FL 33010 - (305) 320-9820

De lunes a viernes, de 9:00 AM a 5:00 PM
(305) 820-0805 | info@xclusiveseniorcare.com | xclusiveseniorcare.com

Venga a conocer cualquiera de los dos centros antes de decidir. Llámenos para agendar una visita.
```

---

## 5. Experience entry

Add one Experience entry so the profile links to the Company Page (step 7 in the README).

| Field | Value |
|---|---|
| Title | `Adult Day Care Center` |
| Employment type | `Self-employed` (or `Full-time` if a person is filling this in) |
| Company name | `Xclusive Senior Day Care Center, Inc` — pick the Page from the dropdown once it exists |
| Location | `Hialeah Gardens, Florida, United States` |
| Start date | `January 2008` |
| Currently working here | Yes |

Description:

```
Licensed adult day care with two locations, serving Hialeah and Hialeah Gardens. Registered nurse on site, one caregiver per four seniors, and a 100% bilingual team.

Daily: therapeutic programs, physical exercise, art, music and horticulture, plus hobbies, crafts and outings. A hot USDA-approved meal and two snacks are served every day, and door-to-door transportation is included.

Most families pay $0 through Medicaid Long-Term Care.

Hialeah Gardens: 12975 W. Okeechobee Rd., Units 2-4, FL 33018 (AHCA #9102), (305) 820-0805.
Hialeah: 705 E 8th Ave., FL 33010, (305) 320-9820.

Both centers are open Monday to Friday, 9:00 AM to 5:00 PM. Call to schedule a tour.
```

---

## 6. Company Page fields

| Field | Value |
|---|---|
| Name | `Xclusive Senior Day Care Center, Inc` |
| LinkedIn public URL | `linkedin.com/company/xclusive-senior-care` |
| Website | `https://xclusiveseniorcare.com` |
| Industry | `Individual and Family Services` |
| Company size | `11-50 employees` |
| Company type | `Privately Held` |
| Year founded | `2008` |
| Logo | `images/profile-picture-400x400.png` |
| Cover image | `images/banner-company-1128x191.png` |

**Tagline** *(limit 120 characters; this is 109)*:

```
Licensed, 100% bilingual adult day care. 2 locations: Hialeah & Hialeah Gardens. RN on site. $0 Medicaid LTC.
```

**Specialties** — add these one at a time:

```
Adult Day Care
Senior Care
Multiple Locations
Medicaid Long-Term Care
Bilingual Care (English/Spanish)
Therapeutic Activity Programs
Supervised Medication
Senior Transportation
Respite Care for Family Caregivers
Memory Care Activities
Nutrition and Meal Service
```

**Locations** — add both:

1. `12975 W. Okeechobee Rd., Units 2-4, Hialeah Gardens, FL 33018` (mark as HQ)
2. `705 E 8th Ave., Hialeah, FL 33010`

Add both. A Page with two locations shows up in LinkedIn searches for either city,
so leaving the second one off costs you the Hialeah searches entirely.

For the Page's About box, reuse the English About from section 3.

---

## 7. Skills

Add these to the personal profile. They are what LinkedIn's search actually matches on:

```
Adult Day Care
Elder Care
Senior Care
Medicaid
Long-Term Care
Home Care
Healthcare Management
Bilingual Communication
Caregiving
Patient Advocacy
Community Outreach
Healthcare Compliance
```

---

## 8. First post — English

Post this the day the profile goes live, so the page has something on it.

```
Our doors have been open in Hialeah and Hialeah Gardens for years, but this is our first day on LinkedIn.

Xclusive Senior Care is a licensed adult day care center with two locations, in Hialeah and Hialeah Gardens. A registered nurse is on site, we keep one caregiver for every four seniors, and our whole team is bilingual.

Every day includes therapeutic programs, physical exercise, art, music and horticulture, a hot USDA-approved meal with two snacks, and door-to-door transportation.

Most families pay $0 through Medicaid Long-Term Care.

If you are caring for a parent and trying to keep your job at the same time, that is exactly the situation we exist for. Call (305) 820-0805 and come see whichever center is closer to you.

#AdultDayCare #SeniorCare #Hialeah #Miami #Caregivers #Medicaid
```

## 9. First post — Español

```
Llevamos años con las puertas abiertas en Hialeah y Hialeah Gardens, pero hoy es nuestro primer día en LinkedIn.

Xclusive Senior Care es un centro de cuidado diurno para adultos con licencia, con dos centros: uno en Hialeah y otro en Hialeah Gardens. Contamos con enfermera registrada en el centro, un cuidador por cada cuatro adultos mayores, y todo nuestro equipo es bilingüe.

Cada día incluye programas terapéuticos, ejercicio físico, arte, música y horticultura, un almuerzo caliente aprobado por USDA con dos meriendas, y transporte puerta a puerta.

La mayoría de las familias pagan $0 a través de Medicaid Long-Term Care.

Si usted cuida a su papá o a su mamá y a la vez trata de mantener su trabajo, esa es exactamente la situación para la que existimos. Llame al (305) 820-0805 y venga a conocer el centro que le quede más cerca.

#CuidadoDeAdultos #AdultosMayores #Hialeah #Miami #Cuidadores #Medicaid
```
